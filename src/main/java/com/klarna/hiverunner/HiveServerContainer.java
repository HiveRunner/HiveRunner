...
public class HiveServerContainer {
    ...
    /*
    There's an obscure bug in HiveRunner where if you have a View query with a Join, it will fail to execute the query
    if the table names after the join are not lowercase, so this method transforms the query so that the parts
